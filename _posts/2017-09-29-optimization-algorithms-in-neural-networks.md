...
categories:
  ...
  - NeuralNetworks
  - MachineLearning
tags: Coursera NeuralNetworks MachineLearning Mini-batch Momentum Softmax
---
...
$$ V_t = \beta\, V_{t-1} + (1 - \beta)\theta_t$$ <br>
$$ V_{100} = <br>
0.1 * \theta_{100} + 0.1 * 0.9 * \theta_{99} <br>
+ 0.1 * (0.9)^2 * \theta_{98} <br>
+ 0.1 * (0.9)^3 * \theta_{97} + ...$$ <br>
...
The names come from the *hard max* approach where a multiclass classification results are mapped to a vector, eg. for being a result of the first class of 4: <br>
$$\begin{bmatrix}
1 \\
0 \\
0 \\
